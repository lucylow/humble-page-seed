 ...
 */
export const parseInvoiceWithAI = async (naturalLanguageInput: string): Promise<any> => {
  try {
 ...
 */
export const convertUSDtoSTX = async (usdAmount: number): Promise<number> => {
  try {
 ...
 */
export const convertSTXtoUSD = async (stxAmount: number): Promise<number> => {
  try {
    const prices = await fetchCoinGeckoPrice(['stacks']);
    const stxPrice = prices.stacks?.usd || 0.85;
