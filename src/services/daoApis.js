 ...
 */
export async function fetchSnapshotProposals(spaceId = 'ens.eth') {
    const query = `
 ...
 */
export async function fetchVotingPower(space, address) {
    const query = `
 ...
 */
export async function getArweaveMetadata(txId) {
 ...
 */
export async function searchArweave(tags) {
    const query = `
    query {
      transactions(
 ...
 */
export async function fetchDAOTreasury(address) {
    try {
        const response = await fetch(`https://api.debank.com/portfolio/token_list?user_addr=${address}`);
        if (!response.ok) {
 ...
 */
export async function fetchTokenHolders(contractAddress, testnet = false) {
    const apiUrl = testnet
        ? 'https://stacks-node-api.testnet.stacks.co'
        : 'https://stacks-node-api.mainnet.stacks.co';
    const response = await fetch(`${apiUrl}/extended/v1/tokens/ft/${contractAddress}/holders`);
    if (!response.ok) {
 ...
 */
export async function fetchCryptoPricesCoinPaprika(coinId = 'btc-bitcoin') {
...
export async function resolveENSName(name) {
    try {
        ...
        const response = await fetch(`https://cloudflare-eth.com/v1/mainnet?method=eth_call&params=[{"to":"0x00000000000C2E074eC69A0dFb2997BA6C7d2e1e","data":"0x3b3b57de${name}"},"latest"]`);
        if (!response.ok)
            return null;
        const data = await response.json();
        return data.result;
 ...
 */
export async function resolveBNSName(name, testnet = false) {
    const apiUrl = testnet
        ? 'https://stacks-node-api.testnet.stacks.co'
        : 'https://stacks-node-api.mainnet.stacks.co';
 ...
 */
export function formatProposalDate(timestamp) {
 ...
 */
export function calculateVotingPercentage(votesFor, votesAgainst) {
 ...
 */
export function isValidArweaveTxId(txId) {
